...
@export var playback : Playback:
	set(value):
		if value != playback:
			...
			if playback:
				playback.track_changed.connect(_on_playback_track_changed.unbind(1))
...
func _track_list_item_get_drag_data(at_position: Vector2) -> Variant:
	...
	if not _selection_echo:
		...
		if at_position:
			var track := _list.get_item_from_position(at_position.y)
			if track:
				data.track = track
...
func _on_playback_track_changed() -> void:
	if playback and source:
		var track := playback.current_track
		var old_track_line : int = -1
		var highlighting := _list.get_highlighting_array()
		if highlighting.size() == 1:
			old_track_line = source.get_tracks().find(highlighting[0])
		
		var new_track_line := source.get_tracks().find(track)
		
		_list.dehighlight_all()
		if new_track_line != -1:
			_list.highlight_item(track)
		
		if old_track_line >= 0 and new_track_line >= 0:
			var begin := _list.scroll_offset
			var end := begin + _list.get_max_lines()
			...
			if visible_befor:
				...
				if absi(track_line_change) == 1:
					_list.scroll_offset += track_line_change
		...
		if _selection_echo:
			if event is InputEventMouseMotion:
				if _list.has_point(event.position):
					accept_event()
					var track := _list.get_item_from_position(event.position.y)
					if track and not track.key in _selection_echo_tracks_keys:
						_selection_echo_tracks_keys[track.key] = true
						if not _list.deselect_item(track):
							_list.select_item(track)
						_list.queue_redraw()
...
func _on_find_line_edit_text_submitted() -> void:
	if not _find.text:
		if _find.has_focus():
			_list.grab_focus()

func _on_find_line_edit_focus_exited() -> void:
	if not _find.text:
		if _find_panel.visible:
			_find_panel.hide()
...
func focus_on_current_track(on_cursor := false) -> void:
	if _list.source and playback and playback.current_track:
...
func get_selection_data_source() -> DataSourceList:
	if not _selection_data_source.get_ref():
		...
		if source:
			selection_ds.source = source.get_root()
